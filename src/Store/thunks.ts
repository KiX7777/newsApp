...
export const getNYFeatured = createAsyncThunk('news/NYTIMES_Featured', async (_, thunkAPI) => {
  try {
...
export const getSearch = createAsyncThunk('news/search', async (query: string, thunkAPI) => {
  const { news } = thunkAPI.getState() as { news: NewsState };
  const page = news.page;

  try {
    const res = axios.get(`/search?query=${query}&page=${page}`);
...
export const getLatest = createAsyncThunk('news/latest', async (_, thunkAPI) => {
  const { news } = thunkAPI.getState() as { news: NewsState };
  const page = news.infiniteScrollPage;

  //call the netlify function on backend and receive data

  try {
    const res = await axios.get(`/latest?page=${page}`);
